...
## Compute a column with the core accruacy for each row.
    num_rows = frame1.shape[0]
    #Copy of the array, cast to a specified type.
    frame1['Score'] = pd.DataFrame({'Score': ['II'] * num_rows}).astype(str)
    for indx in range(num_rows):
        if ((frame1.ix[indx, 'readmitted'] == 'YES') & (frame1.ix[indx, 
            'Scored Labels'] == 'YES')): frame1.ix[indx, 'Score'] = 'TP'
        elif ((frame1.ix[indx, 'readmitted'] == 'NO') & (frame1.ix[indx, 
            'Scored Labels'] == 'NO')): frame1.ix[indx, 'Score'] = 'TN'
        elif ((frame1.ix[indx, 'readmitted'] == 'YES') & (frame1.ix[indx, 
            'Scored Labels'] == 'NO')): frame1.ix[indx, 'Score'] = 'FN'
